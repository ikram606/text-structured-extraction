...
def extraire_competences(texte: str) -> List[str]:
    ...
    patterns_section = [
        r'(?:COMPETENCES?|SKILLS|COMPETENCES?\s+TECHNIQUES?)\s*(?:TECHNIQUES?)?\s*[-:]*\s*\n([\s\S]*?)(?:\n\s*\n[-=]{2,}|\n[-=]{2,}|$)',
        r'[-=]+\s*\n\s*(?:COMPETENCES?|SKILLS|COMPETENCES?\s+TECHNIQUES?).*?\n\s*[-=]+\s*\n([\s\S]*?)(?:\n\s*[-=]{2,}|\n\n[-=]|$)',
    ]
    ...
    if section:
        ...
        for ligne in lignes:
            ...
            if match_cat:
                items = re.split(r'[,;]', match_cat.group(2))
...
def extraire_experience(texte: str) -> List[Dict[str, str]]:
    ...
    patterns_section = [
        r'[-=]+\s*\n\s*(?:EXPERIENCE[S]?\s*PROFESSIONNELLE[S]?|WORK\s*EXPERIENCE|PROFESSIONAL\s*EXPERIENCE).*?\n\s*[-=]+\s*\n([\s\S]*?)(?:\n\s*[-=]{2,}\s*\n\s*[A-Z]|\Z)',
        r'(?:EXPERIENCE[S]?\s*PROFESSIONNELLE[S]?|WORK\s*EXPERIENCE)\s*[-:]*\s*\n([\s\S]*?)(?:\n\s*[-=]{2,}\s*\n\s*[A-Z]|\Z)',
    ]
    ...
    if section:
        ...
        # "Periode : Poste" ou "Periode : Poste\n    Entreprise"
        exp_pattern = r'(?:^|\n)\s*(\w+\.?\s+\d{4}\s*-\s*(?:\w+\.?\s+\d{4}|[Pp]resent|[Aa]ctuel))\s*:\s*(.+?)(?=\n\s*\w+\.?\s+\d{4}\s*-|\Z)'
...
def extraire_donnees_cv(texte: str) -> Dict:
    ...
    Args:
        texte: Le contenu brut du CV au format texte.
